...
3. Revenue by category: Calculate total revenue generated by each product category
'''sql
...
'''
4. Average order value (AOV): Compute the average order value for each customer.
'''sql
...
'''
5. Monthly sales trend: Query monthly total sales over the past year
'''sql
SELECT 
...
'''
6. Customers with no puchases: Find the customer who have registered but never placed an order.
'''sql
...
'''
7. Best selling categories by state: Identify the best-selling product category for each state.
'''sql
WITH ranking_table
AS
...
'''
8. Customer lifetime value (CLTV): Calculate the total value of orders placed by each customer over the lifetime.
'''sql
...
WHERE stock <10;
'''
10. Shipping delays: Identify the orders where the shipping is later than 7 days after the order date
'''sql
...
'''
11. Payment success rate: Calculate the percentage of successful payments across all orders.
'''sql
...
'''
12. Top performing sellers: Find the top 5 sellers based on the total sales value.
'''sql
WITH top_sellers
AS
...
GROUP BY 1, 2;
'''
13. Product profit margin: Calculate the profit margin for each product (difference between price and cost of goods sold)
'''sql
...
GROUP BY 1,2;
'''
14. Most returned products: Query the top 10 products by the number of returns.
'''sql
...
ORDER BY 5 DESC;
'''
15. Inactive sellers: Identify the sellers who haven't made any sales in the last 6 months
'''sql
...
GROUP BY 1;
'''
16. Identify customers into returning or new: If the customer has done more than 5 return categorize them as returning otherwise new 
'''sql
SELECT 
full_name,
total_orders,
...
GROUP BY 1);
17. Top 5 customers by orders in each state : Identify the top 5 customers with the highest number of orders and total sales for each customer
'''sql
SELECT * FROM
(SELECT 
	state, 
...
WHERE rank <=5;
'''
18. Revenue by shipping provider: Calculate the total revenue handled by each shipping provider
'''sql
SELECT 
	shipping_providers,
	COUNT(o.order_id) AS total_orders,
...
19. Top 10 product with highest decreasing revenue compare to last year (2023) and current_year(2024)
'''sql
